...
def test_get_graph_matches_osmnx_node_and_edge_counts(postgis_repo):
    ...
    OSMnx source.
    """
    graph = postgis_repo.get_graph(PLACE)
    assert len(graph.nodes) == 5085
    assert len(graph.edges) == 13850
