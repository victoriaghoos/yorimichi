...
def test_heuristic_never_overestimates_actual_cost_via_networkx(simple_graph, orig, dest):
    """
    Admissibility check through the full networkx integration: for any pair of
    nodes, the heuristic estimate must be <= the actual A*-found path cost.
    """
...
def test_scenic_route_produces_valid_connected_path(simple_graph):
    """S-A* must produce a valid, fully connected route from origin to destination."""
    tree = FakeTree(fixed_distance=0.0)
    weights = np.array([1.0])
    weight_fn = make_edge_weight_fn(simple_graph, tree, weights)
    heuristic_fn = make_heuristic_fn(simple_graph)
...
def test_scenic_route_diverges_from_shortest_path_when_incentivized(asymmetric_graph):
    """When the longer route is scenic enough, S-A* should choose it over the shorter one."""
    ...
    distance_map = {node_2_mid: 0.01, node_3_mid: 0.0}
...
def test_scenic_route_avoids_busy_road_when_alternative_exists(graph_with_busy_road):
    """Even when the primary-road route is shorter, S-A* should prefer the residential-street route."""
    tree = FakeTree(fixed_distance=0.01)
    weights = np.array([1.0])
